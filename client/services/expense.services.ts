...
import { IExpense, IExpenseDashboard, IExpenseForm, IExpenseUpdateForm } from "@/types/expenses"
...
export const updateExpense = async (payload: IExpenseUpdateForm) => {
    const res = await axiosInstance.put(`/api/expenses/${payload.expense_id}`, payload)
    return res.data
}
